...
public final class GameSession {
    ...
    public boolean isPlantSelected(String plantName) {
        return config.selectedPlants()
                .contains(normalizeName(plantName));
    }


    public boolean isPlantBoosted(String plantName) {
        return config.boostedPlants()
                .contains(normalizeName(plantName));
    }
    ...
    public long getRemainingRechargeTicks(
        ...
        long elapsedTicks =
                game.getCurrentTick() - lastTick;

        return Math.max(
                0,
                rechargeTicks - elapsedTicks
        );
    }
