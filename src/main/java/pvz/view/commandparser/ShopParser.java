...
public class ShopParser {
    private static final Pattern BUY_PATTERN = Pattern.compile("^shop\\s+buy\\s+-i\\s+(\\d{1,9})\\s+-n\\s+(\\d{1,9})(?:\\s+-t\\s+(.+))?$");    public Command parse(String input) {
        input = input.trim();
        ...
        if (input.toLowerCase().startsWith("shop buy")) {
            ...
            if (matcher.matches()) {
                ...
                String plantType = matcher.group(3) != null ? matcher.group(3).trim() : null;
