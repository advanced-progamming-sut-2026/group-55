...
public class GreenhouseParser {
    ...
    private final Pattern plantPattern = Pattern.compile("^plant pot at \\((?<x>\\d{1,9}),\\s*(?<y>\\d{1,9})\\)$");
    private final Pattern collectPattern = Pattern.compile("^collect \\((?<x>\\d{1,9}),\\s*(?<y>\\d{1,9})\\)$");
    private final Pattern growPattern = Pattern.compile("^grow \\((?<x>\\d{1,9}),\\s*(?<y>\\d{1,9})\\)$");
