...
public class PlantSelectionMenuParser {
    ...
    private final Pattern upgradePlantPattern = Pattern.compile("^upgrade plant -t (?<plantname>.+)$");
    ...
    private Command tryParseRegexCommands(String input) {
        ...
        if (removeMatcher.matches()) {
            return new PlantSelectionCommand(
                    PlantSelectionCommand.Action.REMOVE_PLANT,
                    removeMatcher.group("plantname")
            );
        }
        ...
        Matcher upgradeMatcher = upgradePlantPattern.matcher(input);
        if (upgradeMatcher.matches()) {
            return new PlantSelectionCommand(
                    PlantSelectionCommand.Action.UPGRADE_PLANT,
                    upgradeMatcher.group("plantname")
            );
        }
    ...
    private Command tryParseFixedCommands(String input) {
        return switch (input) {
            ...
            case "show available plants" -> new PlantSelectionCommand(
                    PlantSelectionCommand.Action.SHOW_AVAILABLE_PLANTS
            );
