...
public class PlantSelectionController extends BaseController {
    ...
    @Override
    protected pvz.model.utils.Message handleSpecificCommand(Command command) {
        if (!(command instanceof PlantSelectionCommand cmd)) return null;
        ...
        switch (cmd.getAction()) {
            ...
            case SHOW_ALL_PLANTS -> {
                ...
                appState.getCurrentUser().getUnlockedPlants()
                        .forEach(p -> view.showSuccess(p.getPlantName()));
            }
            ...
            case ADD_PLANT -> {
                ...
                PlayerPlant playerPlant = appState.getCurrentUser().getOwnedPlant(target);
            ...
            case BOOST_PLANT -> {
                ...
                } else if (!appState.getCurrentUser().spendDiamonds(2)) {
